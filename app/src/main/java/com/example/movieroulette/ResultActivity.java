...
public class ResultActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnSubmitRating.setOnClickListener(v -> {
            if (tmdbId == 0) {
                Toast.makeText(ResultActivity.this, "Movie not loaded yet. Please try again later.", Toast.LENGTH_SHORT).show();
                return;
            }
                ...
                try {
                    ...
                    if (results.length() > 0) {
                        for (int i = 0; i < results.length(); i++) {
                            ...
                            int rating = movie.getInt("vote_average");
                            boolean scoreMatch = false;
                            switch (scorePref) {
                                case "0 - 4.9":
                                    scoreMatch = rating >= 0 && rating <= 4.9;
                                    break;
                                case "5 - 6.9":
                                    scoreMatch = rating >= 5 && rating <= 6.9;
                                    break;
                                case "7 - 7.9":
                                    scoreMatch = rating >= 7 && rating <= 7.9;
                                    break;
                                case "8 - 8.9":
                                    scoreMatch = rating >= 8 && rating <= 8.9;
                                    break;
                                case "9 - 10":
                                    scoreMatch = rating >= 9 && rating <= 10;
                                    break;
                            }
                            if (!movie.has("release_date") || movie.isNull("release_date")) continue;
                            String release_date_str = movie.getString("release_date");
